...
- 反射は **Lambert 拡散 + 軸並行壁の鏡面反射** (`specular`)。一般の BRDF や
  斜めの反射板は未対応。**鏡面壁はどの面でも群落と併用できます**
  (鏡像への直線の z を折り返して群落内の経路長を厳密に測ります)。
  鏡面と遮蔽物・`leafscatter` の併用は未対応で、入力段でエラーにします。
...
**群落との併用** : 鏡面壁は**どの面でも**群落と両立します。鏡映は弧長を
保つので、群落内の経路長は「鏡像への直線の z が群落スラブの逆像に入って
いる割合 × 全長」に厳密に等しくなります。側壁 (x/y 面) の鏡映は z 座標を
保つので逆像はスラブそのもの (直線評価のまま厳密)。床・天井の鏡映は
スラブを動かしますが、鏡像への直線は箱型ビリヤードの展開そのものなので、
実経路の z は周期 `2Lz` の三角波

```
fold(z) = w        (w = z mod 2Lz, w <= Lz)
        = 2Lz - w  (w > Lz)
```

で厳密に得られます (床・天井で何回反射しても成り立ちます)。逆像は
`[zbot + 2kLz, ztop + 2kLz]` と `[2kLz - ztop, 2kLz - zbot]` の和です。
検証は「完全鏡面の壁 = その方向に 2 倍にした幾何」の等価性で縛っており、
側壁 (x 2 倍) も天井 (z 2 倍) も群落込みでビット単位に一致します
(この折り返しを落とすと天井のケースが 8.2% ずれます)。
`leafscatter` だけは鏡像経路ぶんを層へ預け入れる必要があり、まだ群落
散乱とは併用できません (入力段でエラー)。

`ρs` は**波長非依存のスカラ**です。反射フィルムや研磨アルミはほぼ中性
なので、分光にせず 1 個の値で持ちます。

**制限** : 鏡像からの直線は「折り返した経路」を表すので、遮蔽物
(`occluder`) があると遮蔽判定が合いません。
...
| `mirror3.ppfd` 等価 | 鏡面側壁 + 群落 = x 方向 2 倍の幾何 (ビット一致) | 0 | 1e-6 |
| `mirror3.ppfd` 符号 | 鏡を外すと床の平均 PPFD が下がる | — | — |
| `mirror3.ppfd` 拒否 | `leafscatter` 併用は入力段で拒否 | — | — |
| `mirror4.ppfd` 直接光 | 折り返した経路長で E = Φ/(4π)Σ e^{−ks}/d² = 0.7335746 | +0.0001% | 1e-6 |
| `mirror4.ppfd` 等価 | 鏡面天井 + 群落 = z 方向 2 倍の幾何 (ビット一致) | 0 | 1e-6 |
| `mirror4.ppfd` 符号 | 鏡を外すと床の平均 PPFD が下がる | — | — |
| `mirror4.ppfd` 拒否 | `leafscatter` 併用は入力段で拒否 | — | — |
