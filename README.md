...
- 反射は **Lambert 拡散 + 軸並行壁の鏡面反射** (`specular`)。一般の BRDF や
  斜めの反射板は未対応。**鏡面の側壁 (x/y 面) は群落と併用できます**
  (側壁の鏡映は z を保つので群落減衰は直線評価のまま厳密)。鏡面の
  床・天井と群落の併用、鏡面と遮蔽物・`leafscatter` の併用は
  未対応で、入力段でエラーにします。
...
**群落との併用** : 鏡面の**側壁** (x/y 面) は群落と両立します。側壁の鏡映は
z 座標を保つので、z だけで決まる群落スラブは鏡映で不変になり、折り返した
経路の z プロファイルは鏡像への直線のそれと同一 — 群落の減衰・経路長は
直線評価のまま厳密です。検証は「完全鏡面の側壁 = x 方向に 2 倍にした幾何」
の等価性で縛っており、群落込みでビット単位に一致します。鏡面の床・天井は
スラブが鏡映で動くため、`leafscatter` は鏡像経路の預け入れが要るため、
どちらも群落とは併用できません (入力段でエラー)。
...
| `mirror3.ppfd` 拒否 | `leafscatter` 併用と床・天井の鏡面は入力段で拒否 | — | — |
