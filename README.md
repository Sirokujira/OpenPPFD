...
- 群落の散乱光は `leafscatter = on` で戻せます (既定は off = 従来動作)。
  ...
  この散乱場は 1 本の鉛直カラムなので、`leafscatter = on` のときは群落
  スラブを 1 枚しか置けません (減衰だけなら段ごとに置けます)。
...
canopy   = <ztop> <zbot> <LAI> <G> <葉材料名>   # 複数行で多段ラックの各段
...
`canopy` を複数行書くと、段ごとに別々の群落スラブを置けます。段ごとに
高さ範囲・LAI・葉群投影係数 `G` を変えられます (生育ステージの違い)。

```
occluder = shelf 0.0 1.2 0.0 0.6 0.45 0.45 tray   # 段を仕切る棚板
canopy   = 0.70 0.50 1.2 0.5 leaf                 # 上段 (若い株)
canopy   = 0.25 0.05 2.5 0.5 leaf                 # 下段 (収穫期)
```

減衰は段ごとの寄与の和 `τ = Σⱼ Gⱼ aⱼ sⱼ` (`aⱼ` = 葉面積密度、`sⱼ` =
その段を通った長さ) になります。鉛直な光線なら `sⱼ` は各段の厚みなので
`τ = Σⱼ Gⱼ LAIⱼ` です。

**葉材料は全段で共通**です。減衰は `exp(-Σⱼ Gⱼ aⱼ √(1-ωⱼ) sⱼ)` なので、
段ごとに `ω` が違うと波長ごとの重みも段ごとに変わり、経路長を 1 個の
スカラにまとめられなくなります (同じ作物を段ごとに育てるのが多段ラック
なので、変わるのは LAI と高さです)。段ごとに違う葉材料を書くと入力段で
エラーにします。スラブどうしの**重なり**も、同じ葉を二重に数えるので
エラーです (接するのは可)。

`leafscatter = on` の二流散乱場は 1 本の鉛直カラムなので、スラブが
複数あるときは併用できません (複数のカラムがキャビティを介して互いに
やりとりする必要があるため)。
...
**鏡面が 2 面以上あるとき**も正しく数えます。直交する面の鏡映は可換
(`M_zmax M_xmax = M_xmax M_zmax`) なので、「折り返した面の並び」で鏡像を
列挙すると角の鏡像を 2 回数えてしまいます。鏡映群は軸ごとの無限二面体群の
直積なので、鏡像は**軸ごとの展開指数** `n[3]` だけで一意に決まります :

| `n` | 折り返す面 | 座標 |
|---|---|---|
| `0` | なし | `c` |
| `n > 0` | max 面から交互に `n` 回 (xmax, xmin, …) | `n` 偶数 : `c + nL` / 奇数 : `(n+1)L − c` |
| `n < 0` | min 面から交互に `|n|` 回 | 同上 |

重みは交差する面の `Π ρs` です。たとえば `xmax` と `zmax` が完全鏡面で
`specbounce = 2` なら鏡像は 3 個 (x のみ / z のみ / 角) になり、反対側の面が
黒いので 3 段目の経路は消えて打ち切り放射束は厳密に 0 です。**同じ軸の
2 面**が鏡面のときは反射が無限に続くので、`specbounce` で打ち切った分が
`truncated` に出ます。
...
| `canopy2.ppfd` 鉛直 | 2 段の減衰の和 exp(−ΣGⱼLAIⱼ) = exp(−1.55) | −0.0002% | 0.1% |
| `canopy2.ppfd` 斜め | 各段の経路長が 1/cosθ 倍 exp(−1.55/cosθ) | −0.0001% | 0.1% |
| `rack.ppfd` 分割 | 1 枚を 2 枚に割っても結果が変わらない (相互反射込みでビット一致) | 0 | 1e-12 |
| `canopy2.ppfd` 拒否 | スラブの重なり・段ごとに違う葉材料・`leafscatter` 併用は入力段で拒否 | — | — |
...
| `mirror5.ppfd` 鏡像 | 鏡面 2 面なら鏡像は 3 個 : E = Φ/(4π)Σcosθ/d² = 2.7155297 | +0.0000% | 1e-6 |
| `mirror5.ppfd` 打ち切り | 反対側が黒なら 3 段目は消えるので捨てる放射束 = 0 | 0 | 0 |
| `mirror5.ppfd` 収支 | 鏡面 2 面 + 拡散壁でも収支が閉じる | 4.8e-10 | 1e-6 |
| `mirror5.ppfd` 等価 | 鏡面 2 面 = x と z の両方で 2 倍にした幾何 (ビット一致) | 0 | 1e-6 |
