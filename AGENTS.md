...
- **鏡面反射は鏡像法で解く**。軸並行の直方体なので鏡面壁は光源の鏡像と
  ...
  直線は折り返した経路なので、遮蔽物・`leafscatter`・「鏡面の床・天井 +
  群落」の併用は入力段で弾く (黙って誤差を出さない)。鏡面の**側壁**は
  群落と両立する : 側壁の鏡映は z を保つのでスラブは不変、折り返した
  経路の z プロファイル = 鏡像への直線のそれとなり、群落減衰は直線評価の
  まま厳密 (鏡像経路の経路長は直接経路と違うので、拡張形態係数は ff に
  畳み込まず分離保持して各経路長で減衰させる)。壁の吸収は `1 - ρ_d - ρ_s`。拡散壁からの放射が
  鏡面壁で正反射して戻る経路は拡張形態係数 (放射側パッチの鏡像) で運ぶ
  — これを落とすと ρ_s × (鏡面壁への拡散入射) が帳簿から消えて収支が
...
| `mirror3.ppfd` | 鏡面側壁 + 群落 : x 方向 2 倍の幾何の参照解 (`mirror3ref.ppfd`) とビット一致、鏡像の寄与の符号、併用できない組み合わせの拒否 |
