...
- **鏡面反射は鏡像法で解く**。軸並行の直方体なので鏡面壁は光源の鏡像と
  等価で、直接光は厳密になる (近似は段数の打ち切りだけ)。鏡像からの
  直線は折り返した経路なので、遮蔽物・`leafscatter` との併用は入力段で
  弾く (黙って誤差を出さない)。壁の吸収は `1 - ρ_d - ρ_s`。拡散壁からの
  放射が鏡面壁で正反射して戻る経路は拡張形態係数 (放射側パッチの鏡像) で
  運ぶ — これを落とすと ρ_s × (鏡面壁への拡散入射) が帳簿から消えて収支が
  数 % 破れる (実測 -7.7%)。`ρ_d + ρ_s > 1` の材料は入力段で弾く。
- **鏡像は「折り返した面の並び」ではなく軸ごとの展開指数で数える**。
  軸並行の直方体では直交する面の鏡映は可換 (`M_zmax M_xmax = M_xmax
  M_zmax`) なので、面の並びで列挙すると角の鏡像を 2 回数える (実測
  +3.7%、打ち切り放射束も嘘になる)。鏡映群は軸ごとの無限二面体群の
  直積なので、鏡像は `n[3]` (n>0 は max 面から、n<0 は min 面から交互に
  |n| 回) だけで一意に決まる。座標は n が偶数なら `c + nL`、奇数なら
  `(n+1)L - c`。重みは交差する面の `Π ρs`。直接光 (`setup_images`) と
  拡散光 (`spec_transforms`) は同じ列挙を共有する。打ち切り放射束は
  「order = specbounce+1 の変換の重みの総和 × 実光源の放射束」。
  **鏡面が 2 面以上の入力を必ず検証に入れること** — この二重計上は
  鏡面 1 面のケースでは絶対に出ない。
- **鏡面と群落の併用は「鏡像への直線の z を折り返して」測る**。鏡映は
  弧長を保つので、群落内の経路長は「直線の z が三角波 `fold` の逆像に入る
  割合 × 全長」に厳密に等しい。側壁 (x/y) の鏡映は z を保つので `fold` は
  恒等写像、床・天井の鏡映は箱型ビリヤードの展開そのものなので周期
  `2Lz` の三角波になる (`canopy_path`)。ここを素の直線評価で済ませると
  誤差が出る (実測 8.2%)。鏡像経路の経路長は直接経路と違うので、拡張
  形態係数は `ff` に畳み込まず `ffs`/`plens` に分離保持して各経路長で
  減衰させる。散乱の還流 (`leafscatter`) だけは鏡像経路ぶんを層へ預け
  入れる必要があり未対応。
- **群落スラブが複数あるときは「先頭スラブ基準に規格化した経路長」で運ぶ**。
  減衰は `exp(-Σ_j G_j a_j √(1-ω) s_j)` なので、消散係数は先頭スラブの
  `k = G_0 a_0 √(1-ω)` のままにして、段ごとの違いを `w_j = G_j a_j /
  (G_0 a_0)` として経路長側 (`canopy_path` が返す `Σ_j w_j s_j`) に寄せる。
  こうすると内側ループが 1 個のスカラのままで済み、`w_0` は厳密に 1.0 な
  ので 1 枚だけの入力は 1 ビットも変わらない。**葉材料は全スラブで共通**に
  縛る — 段ごとに `ω` が違うと波長ごとの重みが段ごとに変わり、この 1 個の
  スカラにまとめられなくなる (入力段で弾く)。スラブの重なりも二重計上に
  なるので弾く。二流の散乱場は 1 本の鉛直カラムなので、`leafscatter` は
  スラブ 1 枚のときだけ有効。
...
| `canopy2.ppfd` | 多段の群落スラブ : 鉛直・斜めの減衰 exp(−Σ G_j a_j s_j) の解析解、スラブの重なり・段ごとに違う葉材料・`leafscatter` 併用の拒否 (1 枚を 2 枚に割ると `rack.ppfd` の結果がビット一致することも判定する) |
...
| `mirror.ppfd` | 鏡面壁の鏡像法 : 直接 + 鏡像の解析解、鏡面 1 面なら打ち切り誤差 0、鏡面込みの収支、ρs=0 で鏡像が消えること |
| `mirror2.ppfd` | 鏡面 + 拡散の混在 : 拡張形態係数で収支が閉じること、完全鏡面の折り返しが鏡映幾何の参照解 (`mirror2ref.ppfd`) とビット一致すること、ρd+ρs>1 の拒否 |
| `mirror3.ppfd` | 鏡面側壁 + 群落 : x 方向 2 倍の幾何の参照解 (`mirror3ref.ppfd`) とビット一致、鏡像の寄与の符号、`leafscatter` 併用の拒否 |
| `mirror4.ppfd` | 鏡面天井 + 群落 : 折り返した経路長の直接光の解析解、z 方向 2 倍の幾何の参照解 (`mirror4ref.ppfd`) とビット一致、鏡像の寄与の符号、`leafscatter` 併用の拒否 |
| `mirror5.ppfd` | 鏡面 2 面 (直交する鏡映の可換性) : 鏡像 3 個の直接光の解析解、打ち切り放射束 = 0、収支、x と z の両方で 2 倍にした幾何の参照解 (`mirror5ref.ppfd`) とビット一致 |
